...
#define TERRAVCC 33
#define TERRAIN 32 
RTC_DATA_ATTR int terra_value = 0;
...
void setup() {
  ...
  //terra
  digitalWrite(TERRAVCC, HIGH);
  
}

void loop() {
  
  //verifico continuamente la connessione col server, se non è connesso riconnetto tramite funzione. FUNZIONE NON BLOCCANTE: se non si connette continua a provare
  if (!client.connected()) {
    reconnect();
  }
  client.loop();
...
if ( value % 10 != 0){
    terra_value = terra_value + terra;
    Serial.println("sto sommando la terra");
    Serial.print("value= ");
    Serial.println(value);
    Serial.println("terra:");
    Serial.println(terra);
    Serial.println("terra_value:");
    Serial.println(terra_value);
    
  }
  // se invece value è divisibile, è il momeno di fare la media ed inviare i dati al server
  else {
    terra_value = terra_value / 10.0;

    //converto da float a char per poter trasmetterlo in MQTT
    static char terra_char[7];
    dtostrf (terra_value, 6, 2, terra_char);

    //trasmetto al server dati di terra 
    client.publish("serra/terra", terra_char);
    
    Serial.print("terra = ");
    Serial.println(terra_char);
